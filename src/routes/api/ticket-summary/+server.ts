    ...
    if (role === UserRole.ADMIN) {
        query = `
            SELECT 
                ...
                COALESCE(SUM(logs.total_work_items), 0) AS workItems,
                COALESCE(SUM(logs.qualified_work_items), 0) AS workTickets,
        ...
        query = `
            SELECT 
                ...
                teams.name AS teamName,
                -1 AS metricId,
                'Work Tickets' AS qualifiedWorkLabel,
                COALESCE(work_ticket_totals.total_work_items, 0) AS workItems,
                COALESCE(work_ticket_totals.total_work_item_tickets, 0) AS workTickets,
